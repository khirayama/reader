...
import type { User } from '../../node_modules/.prisma/test-client'
...
export class TestAuthService {
  ...
  static async register(email: string, password: string): Promise<AuthResponse> {
    ...
    // パスワードハッシュ化
    const hashedPassword = await TestAuthService.hashPassword(password)
    ...
    return {
      user: TestAuthService.excludePassword(user),
      token,
    }
  }
  ...
  static async login(email: string, password: string): Promise<AuthResponse> {
    ...
    // パスワード検証
    const isValidPassword = await TestAuthService.verifyPassword(password, user.password)
  ...
  static async requestPasswordReset(email: string): Promise<string> {
    ...
    // 新しいリセットトークンを生成
    const token = TestAuthService.generateResetToken()
  ...
  static async resetPassword(token: string, newPassword: string): Promise<string> {
    ...
    // パスワード更新
    const hashedPassword = await TestAuthService.hashPassword(newPassword)
  ...
  static async getUserById(userId: string): Promise<UserResponse | null> {
    ...
    return user ? TestAuthService.excludePassword(user) : null
  }
  ...
  static async changePassword(
    ...
    // 現在のパスワード検証
    const isValidPassword = await TestAuthService.verifyPassword(currentPassword, user.password)
  ...
  static async changeEmail(
    ...
    return TestAuthService.excludePassword(updatedUser)
  }
  ...
  static async createTestUser(email?: string, password?: string): Promise<AuthResponse> {
    ...
    return TestAuthService.register(testEmail, testPassword)
  }
}
