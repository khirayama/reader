...
router.get('/:articleId', requireAuth, async (req, res) => {
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes('権限がありません') ? 403 : 404;
...
router.post('/:articleId/bookmark', requireAuth, async (req, res) => {
  try {
    ...
    const statusCode = error instanceof Error && error.message.includes('権限がありません') ? 403 : 
                      error instanceof Error && error.message.includes('既にブックマーク') ? 409 : 404;
    res.status(statusCode).json({
      success: false,
