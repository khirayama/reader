...
describe('認証API', () => {
  describe('POST /api/auth/register', () => {
    it('有効なデータでユーザー登録が成功する', async () => {
      const response = await request(app).post('/api/auth/register').send(mockUsers.valid)
  ...
  describe('PUT /api/auth/password', () => {
    it('認証されたユーザーがパスワードを変更できる', async () => {
      ...
      const response = await authenticatedRequest(token).put('/api/auth/password').send({
  ...
  describe('PUT /api/auth/settings', () => {
    it('認証されたユーザーが設定を更新できる', async () => {
      ...
      const response = await authenticatedRequest(token).put('/api/auth/settings').send({
        theme: 'DARK',
        language: 'EN',
      })
  ...
  describe('DELETE /api/auth/account', () => {
    it('認証されたユーザーがアカウントを削除できる', async () => {
      ...
      const response = await authenticatedRequest(token).delete('/api/auth/account').send({
      ...
      // 削除後にログインできないことを確認
      const loginResponse = await request(app).post('/api/auth/login').send(mockUsers.valid)
