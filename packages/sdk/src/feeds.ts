...
export class FeedsService {
  ...
  async getFeedById(feedId: string): Promise<Feed> {
    const response = await this.client.get<ApiResponse<Feed>>(`/api/feeds/${feedId}`)
  ...
  async updateFeed(feedId: string, data: UpdateFeedRequest): Promise<Feed> {
    const response = await this.client.put<{ message: string; feed: Feed }>(
      `/api/feeds/${feedId}`,
      data
    )
  ...
  async refreshFeed(feedId: string): Promise<Feed> {
    const response = await this.client.post<{ message: string; feed: Feed }>(
  ...
  async refreshAllFeeds(): Promise<{ success: number; errors: string[] }> {
    const response =
      await this.client.post<ApiResponse<{ success: number; errors: string[] }>>(
